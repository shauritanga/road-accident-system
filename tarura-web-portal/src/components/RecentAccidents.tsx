...
export const RecentAccidents: React.FC<RecentAccidentsProps> = ({ accidents }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border">
      <div className="p-6 border-b">
        ...
        <p className="text-sm text-gray-500">Latest accident reports from the field</p>
      </div>
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center text-sm text-gray-900 mb-2">
                  ...
                  <span>{accident.area}, {accident.district}</span>
                  <span className="mx-2 text-gray-400">•</span>
                  <span className="text-blue-600 font-medium">{accident.region}</span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm text-gray-600 mb-3">
                  ...
                  {accident.involvedParties.length > 0 && (
                    <div className="flex items-center">
                      ...
                      <span>{accident.involvedParties.length} parties involved</span>
                    </div>
                  )}
                </div>

                {/* Environmental Factors */}
                {accident.environmentalFactors && accident.environmentalFactors !== 'None' && (
                  <div className="text-sm text-gray-600 mb-2">
                    <span className="font-medium">Environmental factors:</span> {accident.environmentalFactors}
                  </div>
                )}

                {/* Physiological Issues */}
                {accident.physiologicalIssues && accident.physiologicalIssues !== 'None' && (
                  <div className="text-sm text-gray-600 mb-2">
                    <span className="font-medium">Physiological issues:</span> {accident.physiologicalIssues}
                  </div>
                )}
                ...
                {accident.photoUrls.length > 0 && (
                  <div className="flex items-center text-sm text-gray-600">
                    ...
                    <span>{accident.photoUrls.length} photo{accident.photoUrls.length > 1 ? 's' : ''} available</span>
                  </div>
                )}
              </div>
      ...
      {accidents.length === 0 && (
        <div className="p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">No recent accidents</h3>
          <p className="text-gray-500">No accident reports have been submitted recently.</p>
        </div>
      )}
    </div>
  );
